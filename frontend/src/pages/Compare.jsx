              ...
              <div className="overflow-x-auto no-scrollbar touch-pan-x">
                <table className="w-full text-left border-collapse min-w-[800px]">
                    ...
                    <tr className="bg-slate-900/80 border-b border-white/5">
                      ...
                      <th className="p-8 text-2xl md:text-3xl font-black text-white text-center w-3/8">
                        <div className="flex flex-col items-center">
                          {winnerIndex === 0 ? (
                            <span className="bg-gradient-to-r from-yellow-500 to-amber-500 text-slate-950 text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full mb-3 animate-bounce shadow-[0_0_15px_rgba(234,179,8,0.4)]">
                              🏆 Rating Winner
                            </span>
                          ) : (
                  ...
                  <tbody className="divide-y divide-white/5">
                    ...
                    <ComparisonRow 
                      label="Pros & Cons" 
                      val1={<ProsConsList pros={selectedTools[0].pros} cons={selectedTools[0].cons} />} 
                      val2={<ProsConsList pros={selectedTools[1].pros} cons={selectedTools[1].cons} />} 
                    />
                    <ComparisonRow 
                      label="Use Cases" 
                      val1={<UseCasesList useCases={selectedTools[0].useCases} />} 
                      val2={<UseCasesList useCases={selectedTools[1].useCases} />} 
                    />
                    <ComparisonRow 
...
const ProsConsList = ({ pros, cons }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-md mx-auto text-left">
    <div className="space-y-2">
      <p className="text-[10px] text-emerald-400 font-black uppercase tracking-widest border-b border-emerald-500/10 pb-1 mb-2">Pros</p>
      <ul className="space-y-1">
        ...
        )) : <li className="text-xs text-slate-600 italic">None listed</li>}
      </ul>
    </div>
    <div className="space-y-2">
      <p className="text-[10px] text-red-400 font-black uppercase tracking-widest border-b border-red-500/10 pb-1 mb-2">Cons</p>
      <ul className="space-y-1">
...
const UseCasesList = ({ useCases }) => (
  <div className="flex flex-wrap justify-center gap-1.5 max-w-xs mx-auto">
    {useCases && useCases.length > 0 ? useCases.map((u, i) => (
      <span key={i} className="text-[10px] font-bold px-2 py-0.5 rounded bg-slate-900 text-slate-400 border border-white/5">
        {u}
      </span>
